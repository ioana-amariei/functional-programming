...
Scrieti o valoare Haskell care sa reprezinte expresia aritmetica
"x * (2 + y)". Verificati in ghci ca nu aveti vreo eroare de sintaxa.
-}

valoare = Produs (Var "x") (Suma (Numar 2) (Var "y"))
...
aritmetice care contine potentiale variabile. Deoarece valoarea
...
eval (Numar x) _ = x
eval (Var v) valuatie = valuatie v
eval (Produs e1 e2) valuatie = (eval e1 valuatie) * (eval e2 valuatie)
eval (Suma e1 e2) valuatie = (eval e1 valuatie) + (eval e2 valuatie)

{-
Iata cum putem calcula valoarea expresiei "(x + 10) * 2"
in valuatia val1:
...
Evaluati in ghci expresiile aritmetice
"x * (2 + y)", "(x + 10) * 2" si "(x1 + x2) * x" in
...
{-
O valuatie *partiala* este o functie care asociaza
unei submultimi de variabile un numar intreg.
Reprezentam o valuatie partiala printr-o
functie :: String -> Maybe Int.
De exemplu, valuatia partiala care asociaza variabilei "x"
numarul 10 si variabilei "y" numarul 20 este reprezentata
de urmatoarea functie:
-}
...
Definiti o valuatie partiala care asociaza variabilelor
"x", "y" si "z" numerele 10, 20 si respectiv 30.
-}
...
evalp (Var v) valuatie = valuatie v
evalp (Numar x) _ = Just x
evalp (Suma e1 e2) valuatie = sumaMaybe (evalp e1 valuatie) (evalp e2 valuatie)
evalp (Produs e1 e2) valuatie = produsMaybe (evalp e1 valuatie) (evalp e2 valuatie)
