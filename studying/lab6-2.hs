...
update function variabila valoare = \x -> if x == variabila then valoare else function x
...
executaInstr valuatie (Atrib variabila valoare) = update valuatie variabila (evalp  valoare valuatie)
...
executaProgram valuatie (instructiune:instructiuni) = executaProgram (executaInstr valuatie instructiune) instructiuni
...
evalProgram program = (executaProgram empty program) "result"
