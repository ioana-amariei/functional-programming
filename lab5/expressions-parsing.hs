...
parseExp expression = case parseTerm expression of
						Just(termen, tokenRest) -> case tokenRest of
													(TSuma:tokenRest') -> case (parseExp tokenRest') of
																			Just(e, tokens) -> Just (Suma termen, e)
																			_ -> Just(termen, TSuma: tokenRest')
													l -> Just(termen,l)
...
-- NOTIUNEA DE PUNCT FIX (pe masura ce este procesata lista scade si ajugem fie la lista vida, fie la ceva ce nu putem recunoaste)
-- !!! nu facem full-match => returnez ce reusesc sa parsez si eventual restul
parseFactor (TInt s:rest) = Just (Numar (read s), rest)
parseFactor (TParenLeft:rest) = case parseExp rest of	
									Just (expresie, tokens) -> case tokens of	
																	(TParenRight:tokensRest) -> Just (expresie, tokensRest)
																	_ -> Nothing
									_ -> Nothing								
parseFactor _ = Nothing
